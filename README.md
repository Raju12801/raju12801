# Hi there 👋, I'm Raju Kumar Paswan  

🚀Full Stack Developer | Web Developer | App Developer | AI Enthusiast  

---

## 👨‍💻 About Me  
- 🔭 I’m currently working as an **AIML Trainee** at **NIELIT Chandigarh (Ropar Campus)**  
- 🌱 I’m currently learning **Kotlin** and improving my **Android Development** skills  
- 👯 I’m looking to collaborate on **Application Development & AI-based Projects**  
- 🤔 I’m looking for help with **Dart & Flutter**  
- 💬 Ask me about **App Development, Full Stack Development, Firebase, Kotlin**  
- 😄 Pronouns: **Raju**  
- ⚡ Fun fact: *I just hate being a bad boy 😄*  

---

## 🛠️ Tech Stack  

**Languages:**  
`C` `C++` `Java` `Python` `JavaScript` `Kotlin`  

**Frontend:**  
`HTML` `CSS` `JavaScript` `React.js` `Bootstrap`  

**Backend:**  
`PHP` `MySQL` `REST APIs`  

**Mobile & Cloud:**  
`Android (Kotlin)` `Firebase Auth` `Firebase Realtime Database`  

**AI & CS Fundamentals:**  
`AI/ML Basics` `OOP` `DSA`  

**Tools:**  
`Git` `GitHub` `VS Code` 

---

## 📌 Featured Projects  

### 🏨 Hotel Management System  
**Tech:** HTML, CSS, JavaScript, PHP, MySQL  
- Room booking, check-in/check-out, billing & admin modules  
- Customer feedback system with ratings & reviews  

### 🧠 MindNest – Mental Wellness Android App  
**Tech:** Kotlin, MVVM, Firebase  
- Mood tracking, guided workouts, journals  
- AI-based wellness chatbot  
- Clean MVVM architecture  

### 📚 E-Learning Platform for Coding  
**Tech:** HTML, CSS, JavaScript, PHP, MySQL  
- Courses, video lectures, quizzes  
- Admin dashboard & progress tracking  

---

## 🎓 Education  
- 🎓 **MCA** — Central University of Pondicherry (2024–2026)  
- 🎓 **BCA** — Aryabhatta Knowledge University, Patna (2021–2024)  

---

## 📜 Certifications  
- Artificial Intelligence Development Associate — **NIELIT Chennai** (Pursuing)  
- Android Development — **Udemy** (Pursuing)  
- Full Stack Web Development — **Physics Wallah**  
- The Joy of Computing Using Python — **NPTEL**  
- Generative AI — **VIT**  
- Blockchain Technology & Applications — **IIT Kanpur, IIT Roorkee, MNIT Jaipur**  

---

## 📫 Connect With Me  
- 📧 Email: **rajuraz12801@gmail.com**  
- 💼 LinkedIn: [Raju Kumar Paswan](https://www.linkedin.com/in/raju-kumar-676447378/)  
- 🐙 GitHub: [Raju12801](https://github.com/Raju12801)  

---

⭐ *If you like my work, consider giving a star to my repositories!*  
